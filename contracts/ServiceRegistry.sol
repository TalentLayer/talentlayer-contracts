...
contract ServiceRegistry is AccessControl {
    ...
    function getService(uint256 _serviceId) external view returns (Service memory) {
        require(_serviceId < nextServiceId, "This service does'nt exist");
     ...
     * @param _serviceDataUri token Id to IPFS URI mapping
     */
    function createOpenServiceFromBuyer(string calldata _serviceDataUri)
        public
        returns (uint256)
    {
        uint256 senderId = tlId.walletOfOwner(msg.sender);
        return _createService(Status.Opened, senderId, senderId, 0, _serviceDataUri);
    }
