...
contract TalentLayerPlatformID is ERC721A, AccessControl {
    ...
    /// @param fees the fees generated by the platform
    struct Platform {
        uint256 id;
        string name;
        string dataUri;
     ...
     */
    function mint(string memory _platformName)
        public
        canMint(_platformName)
        onlyRole(MINT_ROLE)
    {
     ...
     */
    function updateRecoveryRoot(bytes32 _newRoot)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function isValid(uint256 _platformId) external view {
        require(
            _platformId > 0 && _platformId <= totalSupply(),
