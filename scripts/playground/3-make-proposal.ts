...
async function main() {
  ...
  await serviceRegistry.connect(bob).createProposal(serviceId, rateTokenBob, 10, bobUri)
  ...
  await serviceRegistry.connect(carol).createProposal(serviceId, rateTokenCarol, 200, carolUri)
}
